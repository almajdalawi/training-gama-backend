...
/**
 * A function provids some payment sevaces such as create user, creagte banck account, deposit, purchase, ...ets
 */
function paymentServace(name) {
    let theUser = new user(name)
    ...
    function bank(cashBalance, creditBalance) {
         ...
         */
        function purchase(theProduct, accountType) {
            if (accountType == null) return 'Please enter the type of the account ("cash" or "credit")'
            ...
            if (accountType.toLowerCase() == 'cash') {
                if (cashBalance >= theProduct.price) {
                    ...
                    return `Payment succeded, you purchased a ${theProduct.name} for $${theProduct.price}, and your cash balance now is $${cashBalance}`
                }
                else return `The cash balance is $${cashBalance}, which is not sufficient to purchase ${theProduct.name}, its price is $${theProduct.price}`
            }
            else if (accountType.toLowerCase() == 'credit') {
                if (creditBalance >= theProduct.price) {
                    creditBalance = creditBalance - theProduct.price
                    return `Payment succeded, you purchased a ${theProduct.name} for $${theProduct.price}, and your cash balance now is $${creditBalance}`
                }
                else return `The credit balance is $${creditBalance}, which is not sufficient to purchase ${theProduct.name}, its price is $${theProduct.price}`
            }
        }

        return {
            purchase, withdraw, deposit, bankStatus, cashBalance, creditBalance, accountId
        }
    }

    return user, bank, theUser
}
...
// Users instances
let mohammad = new paymentServace('Mohammad')
let jehad = new paymentServace('Jehad')
let emad = new paymentServace('Emad')
...
// purchase 
console.log(mohammad.bankAccount.purchase('car', 'cash')) //Product not found, Please enter the product's object!
console.log(mohammad.bankAccount.purchase(car, 'cash')) // Payment succeded, you bought a Car for $12000, and your cash balance now is $2950
console.log(mohammad.bankAccount.purchase(car, 'cash')) // The cash balance is $2950, which is not sufficient to purchase Car, its price is $12000
console.log(emad.bankAccount.purchase(car, 'cash')) // The cash balance is $400, which is not sufficient to purchase Car, its price is $12000
